...
- ❌ Do NOT compute net salaries, rates, grade thresholds, or any financial values on the frontend.
...
- ✅ DO display live UI projections (e.g., the salary preview in the Transaction Drawer) **clearly labeled as estimates only**, understanding the backend's result is always authoritative and final.
...
All data fetching and mutation logic must be encapsulated in dedicated service files (`*.service.ts`). UI components are not allowed to directly invoke `fetch`, `axios`, or any Supabase client function.

**The only valid data flow is:**
```
Component → Service (*.service.ts) → API / Supabase
```

- ❌ Do NOT write `fetch(...)` or `supabase.from(...)` inside a React component.
- ✅ DO create or update a service file (e.g., `employee.service.ts`, `payroll.service.ts`) and call a named function from it.
- ✅ DO keep service functions single-responsibility and clearly named (e.g., `closePayrollRun`, `fetchActiveEmployees`).
...
- ✅ The backend MUST independently validate all inputs: types, required fields, value ranges, and business rules (e.g., one payroll run per month), regardless of what the frontend sends.
...
## Rule 1.4 — TypeScript Strict Mode: No `any`, No Exceptions

The codebase is TypeScript-strict. `any` is banned everywhere.

- ❌ Do NOT use `any`, `as any`, or `// @ts-ignore` to silence type errors.
- ✅ DO use the auto-generated Supabase types (from `generate_typescript_types`) as the foundation for all data models.
- ✅ DO fully type all function signatures, hook return values, component props, and API response shapes.
- ✅ If a type is genuinely unknown, use `unknown` and narrow it with a type guard — do not reach for `any`.

Type safety is not optional. It is your contract between the frontend and the backend.
...
```typescript
// ❌ BAD
const dailyRate = salary / 27;
const grade = avg >= 85 ? "Excellent" : avg >= 70 ? "Good" : "Bad";
```

```typescript
// ✅ GOOD — defined once in src/lib/constants.ts
import { WORKING_DAYS_PER_MONTH, GRADE_THRESHOLDS } from "@/lib/constants";
const dailyRate = salary / WORKING_DAYS_PER_MONTH;
```

All constants with a business meaning (working days, work hours default, grade cutoffs, the 10-day revert window, etc.) must be defined in `src/lib/constants.ts` with a clear name and comment explaining the business rule behind the value.

---

## Rule 1.6 — i18n-Ready: No Hardcoded User-Facing Strings

This app uses `react-i18next` and is RTL-ready. **All user-facing text must go through the translation system.** No exceptions.

- ❌ Do NOT write raw strings directly in JSX (e.g. `<Button>Add Employee</Button>`).
- ✅ DO use the `useTranslation` hook and `t()` function (e.g. `<Button>{t('employees.addEmployee')}</Button>`).
- ✅ New translation keys must be added to both locale files (`en`, `ar`) whenever a new string is introduced.
...
- ✅ The payroll close, revert, and delete operations MUST be fully atomic. If any step fails, the entire operation rolls back.
- ✅ This applies to any future multi-step operation added to the system.

Partial state is not a recoverable condition in a payroll system. Atomicity is required, not preferred.
...
- ✅ DO name files after exactly what they do: `payroll.service.ts`, `employee.types.ts`, `date.utils.ts`, `constants.ts`. The filename should make the contents obvious without opening the file.
