...
const PayrollRunView = () => {
  ...
  const employees = employeeService.getAll().filter(e => !e.isArchived)
  const [transactions, setTransactions] = useState(payrollService.getTransactions())
  ...
  const refresh = () => {
    setTransactions(payrollService.getTransactions())
  }

  const handleCloseMonth = () => {
    payrollService.closeMonth(employees)
  ...
  return (
    <Stack gap="6">
      <HStack justify="flex-end">
        <Button colorPalette="oxygen" onClick={handleCloseMonth}>
...
const PayrollHistoryView = () => {
  const { t } = useTranslation('payroll')
  const history = payrollService.getHistory()
  return (
    <Box>
